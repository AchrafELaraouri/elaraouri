...
// Fonction pour vérifier si l'appareil est un téléphone (largeur d'écran inférieure à 768 pixels)
function isMobile() {
...
function toggleNav() {
    const navbarToggler = document.querySelector('.js-fh5co-nav-toggle');
    const navbarCollapse = document.querySelector('.navbar-collapse');
    const logo = document.querySelector('.navbar-brand'); // Remplacez par la classe ou l'ID de votre logo
...
// Ajoutez un écouteur d'événements pour gérer le clic sur le bouton du menu, uniquement sur les téléphones
document.querySelector('.js-fh5co-nav-toggle').addEventListener('click', function () {
    toggleNav();
});

// Fonction pour mettre à jour la position de l'icône de menu en fonction de la position de défilement (facultatif)
function updateFollowIconPosition() {
    if (isMobile()) {
        const followIcon = document.querySelector('.follow-icon');
        const threshold = 50; // Ajustez cette valeur selon votre préférence
...
// Ajoutez un écouteur d'événements pour mettre à jour la position de l'icône lorsque la page est défilée, uniquement sur les téléphones (facultatif)
...
// Appeler la fonction pour fermer le menu lorsqu'un lien est cliqué
closeMenuOnLinkClick();
// Fonction pour mettre à jour la position du logo en fonction de la position de défilement
...
// Ajoutez un écouteur d'événements pour mettre à jour la position du logo lorsque la page est défilée
window.addEventListener('scroll', function () {
    updateLogoPosition();
});
